...
import signalflag.sdk.auth.check_expiration as check_exp
...
class UsernamePasswordClient(AuthenticatedClient):
    ...
    ``username`` and ``password`` default to the ``SIGNALFLAG_USERNAME`` and
    ``SIGNALFLAG_PASSWORD`` environment variables, falling back to the legacy
    ``RESIM_USERNAME`` and ``RESIM_PASSWORD``.

    The token is cached at ``cache_location``, which defaults to
    ``~/.signalflag/token.json`` (or ``~/.resim/token.json`` when only that
    legacy directory exists); see :func:`signalflag.sdk.auth.const.config_dir`.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Read the environment at construction time rather than import time so
        # variables exported after the import (or set by a test) are honoured.
        if username is None:
        ...
        self._cache_location = (
            default_cache_location() if cache_location is None else cache_location
        )
        self._domain = domain
        self._scope = scope
        ...
        assert username is not None, (
            f"Username is required: pass username= or set {env.describe(env.USERNAME)}"
        )
        assert password is not None, (
            f"Password is required: pass password= or set {env.describe(env.PASSWORD)}"
        )
