...
from signalflag.demo.run import resolve_project, run
from signalflag.sdk.metrics.emissions import Emitter
from signalflag.sdk.test import LogType

# signalflag.demo exports a `run` function, which shadows the `signalflag.demo.run`
# module attribute, so the module has to be fetched by name to patch into it.
run_module = import_module("signalflag.demo.run")
...
CONFIG_PATH = resources.files("signalflag.demo") / "data" / "config.resim.yml"
...
class FakeTest(Emitter):
    """Stands in for signalflag.sdk.test.Test, validating against the shipped config."""
...
class RunTest(unittest.TestCase):
    ...
    def test_urls_point_at_the_app(self) -> None:
        ...
        self.assertEqual(
            urls["dashboard"],
            "https://app.signalflag.ai/projects/project-1/dashboards/dash-1",
        )
    ...
    def test_falls_back_to_the_dashboards_list_when_not_found(self) -> None:
        ...
        self.assertEqual(
            ...
            "https://app.signalflag.ai/projects/project-1/dashboards",
        )
...
class DefaultClientTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        for key in (
            ...
            os.environ.pop(env.legacy_name(key), None)
    ...
    def test_legacy_resim_names_still_work(self) -> None:
        os.environ["RESIM_API_URL"] = "https://api.example.io/v1"
        with patch.object(run_module, "DeviceCodeClient") as client:
            run_module.default_client()
        self.assertEqual(
            client.call_args.kwargs["base_url"], "https://api.example.io/v1"
...
class PackageDataTest(unittest.TestCase):
    ...
    without these they end up poking at ``signalflag.demo.__file__``.
    """
