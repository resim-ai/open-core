...
from signalflag.demo.links import (
    app_base_url,
    batch_url,
    compare_batches_url,
    dashboard_url,
...
class AppBaseUrlTest(unittest.TestCase):
    def test_production_points_at_the_signalflag_app(self) -> None:
        # The production API is still api.resim.ai, but the app has moved to
        # the SignalFlag domain, so the usual prefix swap must not apply.
        self.assertEqual(app_base_url(URL("https://api.resim.ai/v1/")), APP)

    def test_production_ignores_path_and_trailing_slash(self) -> None:
        self.assertEqual(app_base_url(URL("https://api.resim.ai")), APP)
