...
TEST(FuzzHelpersTest, TestOrientedBoxSE3Equal) {
  ...
  const OrientedBoxSE3 box{
      converter::random_element<OrientedBoxSE3>(InOut{rng})};
  OrientedBoxSE3 box_different_reference_from_box{box};
  box_different_reference_from_box.mutable_reference_from_box()->CopyFrom(
      converter::random_element<transforms::proto::SE3>(InOut{rng}));
  OrientedBoxSE3 box_different_extents{box};
  box_different_extents.mutable_extents()->Set(0, -box.extents(0));

  // ACTION / VERIFICATION
  EXPECT_TRUE(converter::verify_equality(box, box));
  EXPECT_FALSE(
      converter::verify_equality(box, box_different_reference_from_box));
  EXPECT_FALSE(converter::verify_equality(box, box_different_extents));
  EXPECT_FALSE(
      converter::verify_equality(box_different_reference_from_box, box));
  EXPECT_FALSE(converter::verify_equality(box_different_extents, box));
}
