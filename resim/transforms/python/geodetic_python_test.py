...
class GeodeticPythonTest(unittest.TestCase):
    ...
    def test_roundtrip(self) -> None:
        ...
        for _ in range(num_tests):
            g = geodetic.Geodetic(
                latitude_deg=np.random.uniform(0.0, 90.0),
                longitude_deg=np.random.uniform(-180.0, 180.0),
                altitude_m=np.random.uniform(0.0, 1e4),
            )

            ecef_position = geodetic.ecef_position_from_geodetic(g)

            round_tripped = geodetic.geodetic_from_ecef_position(ecef_position)
            self.assertAlmostEqual(g.latitude_deg(), round_tripped.latitude_deg())
            self.assertAlmostEqual(g.longitude_deg(), round_tripped.longitude_deg())
            self.assertAlmostEqual(g.altitude_m(), round_tripped.altitude_m())
            self.assertAlmostEqual(g.altitude_ft(), round_tripped.altitude_ft())
    ...
    def test_setters(self) -> None:
        ...
        for _ in range(num_tests):
            ...
            altitude_ft = np.random.uniform(0.0, 3e4)
    ...
    def test_with_rotation_roundtrip(self) -> None:
        ...
        for _ in range(num_tests):
            g = geodetic.GeodeticWithRotation(
                ...
                rotation=so3.SO3.exp(np.random.uniform(-1.0, 1.0, 3)),
            )
            ...
            self.assertAlmostEqual(
                g.geodetic.latitude_deg(), round_tripped.geodetic.latitude_deg()
            )
            self.assertAlmostEqual(
                g.geodetic.longitude_deg(), round_tripped.geodetic.longitude_deg()
            )
            self.assertAlmostEqual(
                g.geodetic.altitude_m(), round_tripped.geodetic.altitude_m()
            )
            self.assertAlmostEqual(
                g.geodetic.altitude_ft(), round_tripped.geodetic.altitude_ft()
            )
